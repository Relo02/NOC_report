% =============================================================================
% Report_metrics.tex — prima riscrittura del report con le metriche vive.
%
% SUPERATO da ../Report_integrated.tex, che lo ha assorbito ed e' il documento
% che si compila. Questo resta come archivio della stesura intermedia, e per
% questo vive qui dentro Metrics/ e non piu' accanto a Report.tex.
%
% Vivendo un livello piu' in basso, i percorsi risolvono da Metrics/: le figure
% da fig/ e ../Images/, la configurazione e la bibliografia da ../. Si compila
% da DENTRO questa cartella:  cd Metrics && latexmk -pdf Report_metrics.tex
%
% I NUMERI NON SONO SCRITTI A MANO. Vengono da metrics_macros.tex e dalle
% tabelle in tab/, generate da  python3 viz/make_results.py  nel repo del
% codice e pubblicate con  python3 viz/publish_tex.py.
% =============================================================================
...
\begin{itemize}
  \item a \emph{discrete} shortest-path program --- an \astar{} search over a Gaussian occupancy grid rebuilt
        from the current light detection and ranging (LiDAR) scan, whose solution is a geometric reference;
...
The stack is a chain of Robot Operating System (ROS)~2 nodes closing the loop between a 3-D LiDAR and the body-frame velocity
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item the \textbf{\astar{} local planner}, which solves the discrete program of Section~\ref{sec:astar} on
        that grid;
  \item the \textbf{nonlinear MPC tracker}, which solves the nonlinear program (NLP) of Section~\ref{sec:nlp} and publishes a
        setpoint;
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    Locomotion layer            & CHAMP gait controller            & AMO policy, Sec.~\ref{sec:g1rl} \\
...
so only within a band, and the G1 profile sets $v_{y,\max}=\resVyMax$~m/s against a forward bound of
$\resVxMax$~m/s --- a ratio of one to two. Earlier campaigns ran that ratio at one to fifteen, with
reversing forbidden on top, and Sections~\ref{sec:l1}, \ref{sec:kkt} and~\ref{sec:robust} recorded what
followed: with no lateral authority it was the input set, not the cost, that decided the motion. The
envelope has since been widened, and the measurements reported here are the run that acted on the
recommendation. Section~\ref{sec:kkt} states the outcome in full.
...
vertical spread is the swing height $h_{\mathrm{sw}}$. A closed-form inverse kinematics (IK) map takes each foot
...
to left, \eqref{eq:impedance} is the point of the architecture: a joint-space proportional--derivative (PD) loop realises a virtual
...
the body frame (the tilt an inertial measurement unit, IMU, provides without a global heading), $q_k,\dot q_k$ the
...
quadruped. The policy is obtained by proximal policy optimization (PPO) \cite{schulman2017ppo}, maximising a
...
deployed G1 profile it is also \emph{vacuous}, since $\nu=\resLagDiscrete$ reduces~\eqref{eq:veldisc} to
...
Second, \textbf{the residual infeasibility on real cycles was a property of $\mathcal{U}$, not of the
data}. On the narrow envelope of the earlier campaigns --- reversing forbidden, lateral speed an order of
magnitude below forward travel --- a demanding $\dsafe$ stayed infeasible even starting from $k=1$: the
robot could only advance along its own heading, and could not back away laterally from a wall of LiDAR
points as fast as it had approached it. That was the interaction between the asymmetry of the admissible
input set and a state constraint.

The deployed envelope now admits $v_x\in[\resVxMin,\resVxMax]$~m/s and $|v_y|\le\resVyMax$~m/s --- a lateral
ratio of one to two, not one to fifteen --- and at that ratio the robot has real lateral authority. The
mechanism above is therefore weaker than it was. How much of the infeasibility survives is not regenerated
by the campaign of Section~\ref{sec:setup} and is not claimed here; what the campaign does measure is
Section~\ref{sec:kkt}, where the collapse of the active set that this section predicted no longer occurs.
...
platform, with $v_x\in[\resVxMin,\resVxMax]$~m/s and $|v_y|\le\resVyMax$~m/s, that region is still
asymmetric about the heading, though far less than on the narrow envelope of the earlier campaigns. The cost is what chooses inside it; the input set is what draws it.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.21\textwidth}p{0.19\textwidth}p{0.52\textwidth}}
    ...
    Input envelope & narrow / \textbf{widened, and it worked} & On the narrow envelope the critical cone
      ...
      Widened to $|v_y|\le\resVyMax$ against $v_{x,\max}=\resVxMax$, the cone no longer collapses:
      $\resConeLast$ dimensions at the last cycle (\S\ref{sec:kkt}, \S\ref{sec:l1}, \S\ref{sec:robust}). \\
...
demanding clearances because no admissible input translated the body sideways (\S\ref{sec:l1}); and the
constraint tightening was inert on the recorded run for the same reason (\S\ref{sec:robust}). A controller
whose admissible set has been reduced to essentially one degree of freedom is not being limited by its
objective, and effort spent on weights is effort spent on the wrong object. The envelope was widened on that
evidence, and the campaign reported here measures the result: the cone holds $\resConeLast$ of $\resNvar$
dimensions at the last sampled cycle and no longer collapses. The diagnosis was right, and it is the one
recommendation of this report that has already been acted on.
