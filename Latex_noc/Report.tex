...
The stack is a chain of ROS~2 nodes (Fig.~\ref{fig:arch}) that closes the loop between a 3-D LiDAR and the
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item a \textbf{mapping module} that rebuilds a local Gaussian occupancy grid from the newest scan at
        $1$~Hz, and maintains two forms of memory, a sparse world-frame cell map with a $20$~s decay and a
        topological navigation graph, whose purpose is precisely to repair the failure mode analysed in
...
only valid for one grid width ahead.
...
two locomotion controllers to the depth needed to justify those two properties and no further --- the
optimization content of the report starts in Section~\ref{sec:formulation}. Throughout, $q\in\R^{n_j}$
...
$h_{\mathrm{sw}}$. The deployed gait uses $h_{\mathrm{sw}}=0.04$~m, $h_{\mathrm{st}}=0.01$~m and a nominal
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    font=\footnotesize,
    >={Latex[length=1.7mm]},
    blk/.style={draw, rounded corners=2pt, align=center, inner sep=3.5pt, fill=black!3},
    hlblk/.style={draw=bluePoli, line width=0.9pt, rounded corners=2pt, align=center, inner sep=4pt,
                  fill=bluePoli!10},
    sig/.style={->, semithick},
    sl/.style={font=\scriptsize, inner sep=1.6pt},
  ]
    % ------------------------------------------------------------------ planning
    \node[blk,  text width=44mm] (astar) at (0,4.4)  {\astar{} search on the occupancy grid\\$1$~Hz};
    \node[hlblk,text width=44mm] (mpc)   at (0,2.75) {nonlinear MPC tracker\\$10$~Hz};

    \draw[sig] (0,5.55) -- node[sl,right] {goal $x_g$} (astar.north);
    \draw[sig] (astar) -- node[sl,right] {$x^{\mathrm{ref}}$} (mpc);

    % --------------------------------------------------------- locomotion layer
    \coordinate (split) at (0,1.35);
    \node[blk, text width=52mm] (champ) at (-3.5,0.35)
    ...
    \node[blk, text width=52mm] (rl) at (3.5,0.35)
      {\textbf{G1}: AMO reinforcement-learning policy\\
       $\mathcal{M}\to\pi$~\eqref{eq:rlpolicy} $\to$ PD setpoint~\eqref{eq:rlpd}\\
       $\to$ joint PD, $50$~Hz};
    \node[draw, dashed, rounded corners=3pt, fit=(champ)(rl)(split), inner sep=4pt] (llbox) {};
    \node[sl, anchor=south west] at ([xshift=1pt]llbox.north west)
      {low-level locomotion layer (platform-specific)};

    \draw[sig] (mpc.south) -- node[sl, right, align=left]
      {$u=(v_x,v_y,\omega)^{\mathrm{cmd}}$\\ first-order lag~\eqref{eq:lagcont}} (split);
    \draw[sig] (split) -| (champ.north);
    \draw[sig] (split) -| (rl.north);

    % ------------------------------------------------------------------- plant
    \node[blk, text width=26mm] (robot) at (0,-1.9) {robot $+$ ground};
    \draw[sig] (champ.south) -- (robot.north west);
    \draw[sig] (rl.south)    -- (robot.north east);
    \node[sl] at (0,-1.15) {$\mathcal{T}$: joint torques};

    % ---------------------------------------------------------------- feedback
    \node[blk, text width=38mm] (est)  at (5.8,2.75) {state estimation (EKF)\\$\hat x$: pose $+$ body twist};
    \node[blk, text width=38mm] (grid) at (5.8,4.4)  {Gaussian occupancy grid\\$\mathcal{P}_k$, rebuilt at $1$~Hz};

    \draw[semithick] (robot.east) -- node[sl, above] {LiDAR, IMU, encoders} (9.0,-1.9) -- (9.0,4.4);
    \draw[sig] (9.0,2.75) -- (est.east);
    \draw[sig] (9.0,4.4)  -- (grid.east);
    \draw[sig] (est.west)  -- (mpc.east);
    \draw[sig] (grid.west) -- (astar.east);
  \end{tikzpicture}
  \caption{Cascaded structure of the controller, from the goal down to joint torques. The report analyses the
  two stages at the top; everything inside the dashed box is a platform-specific locomotion layer that the MPC
  sees only through the first-order lag~\eqref{eq:lagcont}, which is the abstraction seam of the design. The
...
\begin{equation}
  ...
  \label{eq:grid}
\end{equation}
with $\Phi$ the standard normal CDF and $\sigma=0.15$~m. Equation~\eqref{eq:grid} is an \emph{inflation}
...
the obstacle halo. Cells with $P(c)\ge P_{\mathrm{thr}}=0.4$ are hard obstacles; note that
$1-\Phi(1)\approx0.159$ and $1-\Phi(0.25)\approx0.40$, so the hard boundary sits at
$d_{\min}\approx0.25\sigma=0.0375$~m of a return --- the effective body clearance is produced by the
\emph{soft} cost of Section~\ref{sec:astar}, not by the threshold. The left panel of
...
\begin{equation}
  ...
  \label{eq:kincont}
\end{equation}
The identified constants are $\tau_v=0.12$~s and $\tau_w=0.10$~s.\footnote{In the deployed code the lateral
...
function of $\tau_v$. At the identified $\tau_v=0.12$~s the instantaneous model mispredicts the horizon
...
Section~\ref{sec:localmin} shows the resulting livelock in a two-room world, which is exactly what the
persistent cell map and the navigation graph of Section~\ref{sec:overview} exist to prevent.
...
already solving that problem globally on the grid. The effort weights rise by an order of magnitude, which
buys smoothness. The six greyed weights --- among them every tracking weight and the terminal multiplier ---
...
\begin{itemize}
  \item the ROS~2 cascade (setpoint low-pass filter and proportional \texttt{/cmd\_vel} controller) is
        ...
        implementation;
  \item the persistent cell map and the navigation graph are \emph{disabled}, so the planner sees only the
        current scan. This is the worst case for the rolling-horizon rule~\eqref{eq:localgoal} and it is what
        makes the failure mode of Section~\ref{sec:localmin} observable.
\end{itemize}
...
arc. Adding memory is therefore not a refinement but a requirement, which is exactly what the deployed stack
does with the $20$~s persistent cell map and the topological navigation graph disabled in this harness. This
is the clearest illustration in the whole study of the difference between solving an optimization problem well
and posing the right one.
...
$0.0977$~m; the value shipped in the deployed \texttt{planner\_params.yaml} is $0.098$~m. The $\aobs$ sweep
...
search ranges of Table~\ref{tab:params} are themselves a design choice that deserves revisiting.
...
and the non-negativity of $v_x$, produces the livelock of Section~\ref{sec:localmin}. The deployed stack works
around it with a decaying persistent map and a topological graph. A cleaner formulation would put the local
target inside the optimization --- for instance a receding-horizon target chosen to minimise a
value function learned or computed on the graph --- rather than fixing it by a ray intersection before the
NLP is even built.
...
correctly. Posing the right problem remains harder than solving it well.
...
$N$ to $20$ and $M$ to $8$ to recover the resulting computational headroom on the onboard computer; and
replace the ray-based local target with one that respects the topology of the free space, so that the quality
of the solution matches the quality of the solver.
...
\texttt{src/a\_star\_mpc\_planner/} \texttt{config/planner\_params.yaml}. Animated versions of
