...
reference into dynamically feasible velocity commands. This report analyses the map-free navigation stack of
the \texttt{Go2\_navigation} project from exactly that optimization viewpoint. The plant is a legged robot
abstracted as a body-frame holonomic agent with first-order actuator lag, yielding a six-state, three-input
discrete model; the nonlinear MPC tracker built on it is a $456$-variable NLP with $306$ equality constraints,
$1224$ smooth obstacle-barrier terms and a $5$~s horizon, formulated in CasADi and solved by IPOPT at
...
cycle \cite{rawlings2017mpc}. The price is computational: a nonlinear program must be solved to sufficient
accuracy inside the sampling period, every period, on the robot.

This report analyses the navigation stack of the \texttt{Go2\_navigation} project\footnote{Companion
repository: \url{https://github.com/talos-robotics-ai/Go2_navigation}. The stack is deployed on a Unitree Go2
quadruped, whose CHAMP locomotion controller \cite{champ} exposes a body-frame velocity interface; the same
code has since been ported to a Unitree G1 humanoid, which is what makes the ``robot-agnostic'' claim
testable.} from the point of view of the course, i.e.\ as a numerical optimization problem. Two programs are
...
\begin{enumerate}[label=\arabic*.]
  \item an \textbf{odometry bridge} that converts the EKF pose estimate into a single pose topic and
        differentiates it, with an exponential filter, into a body-frame velocity estimate (the robot does
  ...
  \item a \textbf{proportional velocity controller} at $20$~Hz that turns the setpoint into
        \texttt{/cmd\_vel}, and a safety layer that gates it.
\end{enumerate}
...
velocity-tracking device with a time constant: the report never models legs, contacts or gait, and the
quadruped is abstracted as a planar holonomic agent. Second, the map is \emph{local and volatile}. The
